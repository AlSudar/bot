...
async def show_voyah_subsections(query: CallbackQuery, context: ContextTypes.DEFAULT_TYPE):
    ...
    [InlineKeyboardButton("Почему подключение автомобиля к мобильному приложению платное?", callback_data=json.dumps({"s": 'mobile_app', "t": 'sub_voyah_7'}))],
...
async def handle_action_voyah(query: CallbackQuery, action_type: SectionData, context: ContextTypes.DEFAULT_TYPE, callback: Callable[
    ...
    'sub_voyah_6': {
        ...
        'question': 'Кто ставит оборудование для подключения к мобильному приложению?'
    },
    'sub_voyah_7': {
        ...
        'question': 'Почему подключение автомобиля к мобильному приложению платное?'
    },
    'sub_voyah_8': {
