...
async def handle_action_simcard(query: CallbackQuery, action_type: SectionData, context: ContextTypes.DEFAULT_TYPE, callback: Callable[
    ...
    'simcard_balance': {
        ...
        'simcard_balance': {
        'text': f'Напишите нашей поддержке {HELPER_BOT_USERNAME}  и укажите VIN автомобиля. Мы постараемся уточнить номер SIM - карты установленной на Вашем VOYAH и предоставим ответ.',
        'question': 'Как узнать номер SIM - карты для пополнения?'
    },
    ...
    'smcrd_error_bfr_wrk_yes': {
        'text': 'Требуется пополнить баланс SIM - карты, как это осуществить см. в разделе "SIM"',
